...
class ImageGenerationClientTest < ActiveSupport::TestCase
  ...
  test "it raises when the API responds with a 4XX error" do
    image_generation_request = stub_request(:post, ENV["IMAGE_GENERATOR_API_URL"]).and_return(status: 401)
  ...
  test "it raises when a response contains an error field" do
    image_generation_request = stub_request(:post, ENV["IMAGE_GENERATOR_API_URL"]).and_return(
      body: {
        error: {
          message: "Gone bad"
        }
